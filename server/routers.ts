...
export const appRouter = router({
  ...
  github: router({
    ...
    configureRefresh: protectedProcedure.input(z.object({ refreshMinutes: z.number().int().min(15).max(1440), healthThreshold: z.number().int().min(0).max(100) })).mutation(async ({ ctx, input }) => {
      ...
      // Heartbeat runs at a fixed safe cadence; the callback gates by refreshMinutes.
      // This avoids approximating intervals such as 90 minutes with an incorrect cron step.
      const job = await createHeartbeatJob({ name: `github-refresh-${ctx.user.id}`, cron: "0 */15 * * * *", path: "/api/scheduled/github-refresh", description: "Refresh GitHub project health metrics" }, sessionToken);
