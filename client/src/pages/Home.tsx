...
function SettingsPage({ userName, setUserName, password, setPassword, logout }: { userName: string; setUserName: (value: string) => void; password: string; setPassword: (value: string) => void; logout: () => void }) { const [nameDraft, setNameDraft] = useState(userName); const [passwordDraft, setPasswordDraft] = useState(password); const [provider, setProvider] = useState("Custom API"); const [endpoint, setEndpoint] = useState("https://api.example.com/v1"); const [apiKey, setApiKey] = useState(""); const [models, setModels] = useState<string[]>(["venture-reasoner", "venture-creative", "venture-fast"]); const [selectedModel, setSelectedModel] = useState("venture-reasoner"); const [fetchingModels, setFetchingModels] = useState(false); const [apiState, setApiState] = useState("لم يتم الاختبار"); const fetchModels = () => { setFetchingModels(true); setApiState("جارٍ جلب النماذج..."); setTimeout(() => { setModels(["venture-reasoner", "venture-creative", "venture-fast", "venture-vision"]); setSelectedModel("venture-reasoner"); setFetchingModels(false); setApiState("تم جلب 4 نماذج تجريبية"); toast("تم تحديث قائمة النماذج تجريبيًا"); }, 700); }; return <><div className="page-heading"><div><p className="eyebrow">{pageMeta.settings.eyebrow}</p><h1>{pageMeta.settings.title}</h1><p className="page-description">{pageMeta.settings.description}</p></div><button className="outline-button" onClick={logout}><UserRound size={15} /> تسجيل الخروج</button></div><div className="settings-grid"><section className="panel settings-card"><div className="panel-head"><div><p className="eyebrow">Profile</p><h2>بيانات المستخدم</h2></div><div className="settings-icon"><UserRound size={18} /></div></div><label>اسم المستخدم<input value={nameDraft} onChange={(e) => setNameDraft(e.target.value)} /></label><label>وصف قصير<textarea defaultValue="مطور تطبيقات وألعاب موبايل" /></label><button className="primary-button small" onClick={() => { setUserName(nameDraft); toast("تم حفظ بيانات المستخدم تجريبيًا") }}><Check size={14} /> حفظ البيانات</button></section><section className="panel settings-card"><div className="panel-head"><div><p className="eyebrow">Security</p><h2>كلمة المرور</h2></div><div className="settings-icon"><Settings2 size={18} /></div></div><p className="settings-note">الدخول في هذا الـ Prototype يعتمد على كلمة مرور فقط. في النسخة الحقيقية سيتم حفظها بشكل آمن داخل Backend.</p><label>كلمة المرور الجديدة<input type="password" value={passwordDraft} onChange={(e) => setPasswordDraft(e.target.value)} /></label><button className="primary-button small" onClick={() => { setPassword(passwordDraft); toast("تم تغيير كلمة المرور تجريبيًا") }}><Check size={14} /> تحديث كلمة المرور</button></section><section className="panel settings-card api-settings-card"><div className="panel-head"><div><p className="eyebrow">AI Provider</p><h2>Custom API للذكاء الاصطناعي</h2></div><div className="settings-icon"><Bot size={18} /></div></div><label>اسم المزود<input value={provider} onChange={(e) => setProvider(e.target.value)} placeholder="OpenAI-compatible API" /></label><label>API Endpoint<input value={endpoint} onChange={(e) => setEndpoint(e.target.value)} placeholder="https://.../v1" /></label><label>API Key<input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="••••••••••••" /></label><div className="api-actions"><button className="outline-button small" onClick={fetchModels} disabled={fetchingModels}>{fetchingModels ? <><Loader2 size={14} className="spin" /> جارٍ الجلب</> : <><RefreshIcon /> جلب النماذج</>}</button><span className="api-state">{apiState}</span></div><label>النموذج المستخدم<select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)}>{models.map((model) => <option value={model} key={model}>{model}</option>)}</select></label><button className="primary-button small" onClick={() => toast(`تم حفظ إعدادات ${provider} والنموذج ${selectedModel} تجريبيًا`)}><Check size={14} /> حفظ إعدادات الذكاء الاصطناعي</button></section></div></> }
