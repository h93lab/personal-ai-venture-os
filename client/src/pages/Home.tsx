...
function AppShell({ active, setActive, children }: { active: PageKey; setActive: (key: PageKey) => void; children: React.ReactNode }) {
  ...
  return <div className="app-shell" dir="rtl">
    <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
      ...
      <div className="sidebar-bottom"><div className="sidebar-status"><span className="live-dot" /><div><strong>المساعد متصل</strong><span>آخر مزامنة منذ 3 د</span></div></div><button className="nav-item" onClick={() => toast("الإعدادات ستكون متاحة في النسخة التالية") }><Settings2 size={17} /><span>الإعدادات</span></button></div>
    </aside>
...
export default function Home() {
  ...
  const content = useMemo(() => { if (active === "overview") return <Overview go={setActive} />; if (active === "tasks") return <Tasks tasks={tasks} setTasks={setTasks} />; if (active === "discovery") return <Discovery />; if (active === "ideas") return <Ideas />; if (active === "projects") return <Projects />; return <SimplePage page={active} />; }, [active, tasks]);
  return <AppShell active={active} setActive={setActive}>{content}<footer className="prototype-footer"><span><CircleDot size={12} /> Prototype · بيانات تجريبية فقط</span><span>Personal AI Venture OS <span className="footer-divider">·</span> v0.1</span></footer></AppShell>;
}
