...
export default function Home() {
  ...
  const content = useMemo(() => { if (active === "overview") return <Overview go={setActive} />; if (active === "tasks") return <Tasks />; if (active === "discovery") return <Discovery />; if (active === "ideas") return <Ideas />; if (active === "projects") return <Projects />; if (active === "settings") return <SettingsPage userName={userName} onProfileUpdated={() => { void refresh(); }} logout={logout} />; if (active === "knowledge") return <Knowledge />; if (active === "validation") return <ValidationLab />; if (active === "briefs") return <ProductBrief />; if (active === "health") return <ProjectHealth />; return <SimplePage page={active} />; }, [active, tasks, userName, logout]);
  ...
  if (error || !isAuthenticated) return <div className="app-loading" role="alert"><AlertTriangle size={24} /><span>تعذر التحقق من الجلسة. أعد تحميل الصفحة أو سجّل الدخول من بوابة الحساب.</span></div>;
